...
싱글턴을 구현할 때 멀티스레딩 환경에서도 동작가능하게끔 구현해야한다. 그러려면 동시성문제를 생각해야하는데, 객체를 생성해주는 메서드에 동기화 블럭을 지정하는 방법이 있을 수 있다. 코드는 아래와 같다.

> 만약 동기화 블럭을 지정하지 않는다면, 객체가 변조될 것이다. 왜냐하면 static 키워드가 붙은 애들은 메서드(클래스) 영역에 메모리가 등록되는데, 이 영역은 힙 영역과 마찬가지로 공유 가능한 자원이기 때문이다.
...
LazyHolder 방식은 static 객체임에도 불구하고 필요시 메모리에 할당해서 쓴다는 것과(동적바) 그 방식이 쓰레드 안전성을 보장해준다는 다.
...
하나의 요청(Request)를 처리하기 위해서는 데이타 액세스 로직(DAO), 서비스 로직, 비즈니스 로직, 프리젠테이션 로직등 다양한 기능을 담당하는 오브젝트들이 계층형 구조로 이루어 진다. 매번 클라이언트 요청이 올때마다, 각 로직을 담당하는 객체를 새로 만들어 사용한다면, 아무리 Java가 오브젝트 생성과 GC(Garbage Collection) 성능이 좋아졌어도 서버가 부하를 감당하기 들다. 이 때문에 엔터프라이즈 분야에서는 서비스오브젝트 라는 개념을 사용해 왔는데 Servlet 은 Java 엔터프라이즈 기술의 가장 기본이 되는 서비스 오브젝트라고 할수 있다. Servlet 은 대부분 멀티스레드 환경에서 Singleton 으로 동작한다. 즉, Servlet 클래스 당 하나의 오브젝트만 만들어 주고, 사용자의 요청을 담당하는 여러 스레드에서 하나의 오브젝트를 공유해서 사용한다.

스프링은 어노테이션 설정만으로 IoC 컨테이너에 제어권을 넘겨줌으로써 손쉽게 사용할 수 있다.
...
private 생성자를 가진 클래스도 스프링의 빈으로 바로 사용이 가능하다 이다. 스프링은 리플렉션을 통해서 인스턴스를 만들고, 리플렉션을 통해서라면 private 생성자를 호출해서 인스턴스를 만드는 것이 가능하다. 해보면 알겠지만 스프링은 별 경고 없이 그냥 빈을 만들어버린다.
...
따라서 Spring 에서 singleton 이 멀티스레드 환경에서 서비스 오브젝트에 사용되는 경우에는 상태정보를 내부에 갖고 있지 않은 stateless 로 만들어야 한다. 이 경우, 각 요청에 대한 정보나 생성된 정보는 메서드 파라미터나 메서드 안의 로컬변수 혹은 리턴값 등으로 처리해야 한다. __싱글톤이라해도 메서드 파라미터나, 메서드 안에서 생성되는 로컬변수는 메서드가 호출될 때마다 매번 새로 할당되므로__ 싱글톤이라 해도 여러 스레드가 변수의 값을 덮어쓸 일은 없다.
